...
import { GetPools } from '@app/api/Queries/Getters'
...
const Pool = () => {
    const [farmToApprove, setFarmToApprove] = useState<PoolType | null>()
    ...
    const updateAmountFN = async (pool: PoolType, shouldRefetch = false) => {
        try {
            const res = await updateAmount(pool, 'pools')
    ...
    return (
        <>
            <RoiModal
                open={isRoiOpen}
                close={() => setOpenRoi(false)}
                onGetAM={() => setOpenDeposit(true)}
            />
            <DepositModal
                open={isDepositOpen}
                close={() => setOpenDeposit(false)}
                onConfirm={(amount) => updateAmountFN({ ...farmToApprove as PoolType, approved_amount: amount, approved: true }, true)}
            />
